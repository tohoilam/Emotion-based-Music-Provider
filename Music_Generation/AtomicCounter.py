...
def usageExample():
  with Pool(4) as pool:
    ...
    elements = []
    manager = Manager() # For sharing resources inside threads
    print("START")
    counter = AtomicCounter(manager, len(elements)) # For sharing counter between threads
    results = pool.starmap(_process, zip(elements, cycle([counter]))) # Laund the process mehthod on the elements list
